...
red_wine <- read_delim("https://raw.githubusercontent.com/rfamg/Big-Data-II-Spring-2017/master/winequality-red.csv", 
...
ind <- sample(2,nrow(red_wine),replace=TRUE, prob=c(0.6,0.4)) 
head(as.data.frame(ind), 20) # show first 20 values of ind



train <- red_wine[ind==1,]  # new df "train" will consists of randomly 
...
lm <- lm(quality ~ alcohol + sulphates, data=train)
summary(lm)
...
lm2 <- lm(quality ~ alcohol + sulphates, data=test)
summary(lm2)
